...
NbSubject = 1;
RunsToCreate = [1 3 5];
FirstConditionAllocation = [1 2 2 1 1 2];
NbStimPerBlock= 13;
...
for SubInd = 1:NbSubject
    ...
    for RunNb = 1:length(RunsToCreate)
        ...
        for h=1:2
            ...
            if AttentionCondition == 1
                AttentionCondition = 2;
                NeglectCondition = 1;
            else
                AttentionCondition = 1;
                NeglectCondition = 2;
            end
            
            if h==1
                TrialsFinal = [TrialsFinal ; AttentionCondition]; 
            end
...
for SubjInd = 2:NbSubject
    ...
    for FileInd=1:size(FilesList,1)
        if mod(SubjInd,2)~=0
            copyfile(fullfile(StartDirectory, 'Subject_1', FilesList(FileInd).name), ...
